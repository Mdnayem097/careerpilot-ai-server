...
class GeminiService {
  ...
  private init() {
    ...
    if (apiKey && apiKey !== 'your_gemini_api_key_here') {
      ...
      console.warn('⚠️ GEMINI_API_KEY is not set or using placeholder in .env. Operating in fallback mode.');
    }
  }
   ...
   */
  public async analyzeResume(rawText: string, targetRole: string): Promise<ResumeAnalysisResult> {
    if (this.genAI) {
      try {
...
`;
        ...
        console.error('[Gemini API Error - analyzeResume]:', err.message || err);
      }
    }
...
`;
        ...
        console.error('[Gemini API Error - generateCareerRecommendation]:', err.message || err);
      }
    }

    return this.fallbackCareerRecommendation(currentRole, targetRole, pacing, memoryContext);
  }
...
`;
  ...
  private fallbackAnalyzeResume(rawText: string, targetRole: string): ResumeAnalysisResult {
    const requiredKeywords = [
      'TypeScript', 'React', 'Next.js', 'Node.js', 'Express.js', 'MongoDB', 'System Design', 'REST API', 'Agentic AI'
    ];
    const lowerRaw = rawText.toLowerCase();
    const extractedSkills = requiredKeywords.filter((kw) => lowerRaw.includes(kw.toLowerCase()));
    const missingKeywords = requiredKeywords.filter((kw) => !lowerRaw.includes(kw.toLowerCase()));
    const atsScore = Math.min(Math.round((extractedSkills.length / requiredKeywords.length) * 70 + 25), 95);
      ...
      improvementSuggestions: [
        {
          ...
          issue: missingKeywords.length > 0 ? `Missing key keywords: ${missingKeywords.slice(0, 3).join(', ')}` : 'Needs more quantified impact metrics.',
          recommendation: 'Incorporate missing keywords into impact-focused bullet points.',
      ...
      milestones: [
        {
          ...
          skillsToAcquire: ['TypeScript', 'Next.js 15 App Router', 'Express.js REST APIs'],
          actionItems: ['Build full-stack production application with MongoDB persistence.'],
          completed: false
        },
        {
          phase: 'Phase 2: Advanced Systems & AI Integration',
          timeframe: 'Month 3 - Month 4',
          skillsToAcquire: ['Agentic AI Architecture', 'Gemini API', 'Vector DBs'],
          actionItems: ['Implement multi-agent workflows and structured JSON tool calls.'],
          completed: false
        }
      ],
  ...
  private fallbackChatResponse(
    ...
    return {
      content,
      agentThinking: '[Fallback Engine] Generated structured career recommendation.'
    };
  }
}
